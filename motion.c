...
int m_eof(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	return m_line_goto(to, repeat, true, win);
}

int m_eol(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	list_t *l = list_seek(win->buf->head, from->y, false);
...
int m_sos(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	to->y = UI_TOP(win);
...
int m_eos(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
...
int m_mos(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
...
int m_goto(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	if(m->pos.y > -1)
...
int m_aggregate(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	*to = *from;
	for(int i = 0; i < 2; i++){
		motion *entry = m->aggregate[i];
		if(entry->func(&entry->arg, repeat, win, to, to) == MOTION_FAILURE)
...
int m_move(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	to->y += m->pos.y * DEFAULT_REPEAT(repeat);
	to->x += m->pos.x * DEFAULT_REPEAT(repeat);
...
int m_sof(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	return m_line_goto(to, repeat, false, win);
}

int m_sol(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	list_t *l = list_seek(win->buf->head, from->y, false);
	unsigned int i;
...
static int m_linesearch(
		motion_arg const *arg, unsigned repeat, window *win,
		point_t const *from, point_t *to,
...
int m_para(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	return m_linesearch(m, repeat, win, from, to, m_search_para, NULL);
}
...
int m_func(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	int r = m_linesearch(m, repeat, win, from, to, m_search_func, &m->scan_ch);
	if(r == MOTION_SUCCESS)
		to->x = 0;
	return r;
}
...
int m_word(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
...
int m_findnext(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	return m_findnext2(
			last_find_ch,
...
int m_find(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	bool raw;
	return m_findnext2(
...
int m_searchnext(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	if(!lastsearch){
...
int m_search(motion_arg const *m, unsigned repeat, window *win,
		point_t const *from, point_t *to)
{
	const bool forward = m->i > 0;
	...
	return m_searchnext(&(motion_arg){ 1 }, repeat, win, from, to);
}

int m_visual(
		motion_arg const *arg, unsigned repeat,
		window *win, point_t const *from, point_t *to)
{
	(void)repeat;
...
int motion_apply_win_dry(
		const motion *m, unsigned repeat, window *win,
		const point_t *from, point_t *out)
{
	*out = *from;
	int r = m->func(&m->arg, repeat, win, from, out);
	if(r == MOTION_SUCCESS){
		if(out->y < 0 || out->x < 0)
			r = MOTION_FAILURE;
	}
...
int motion_apply_win(const motion *m, unsigned repeat, window *win)
{
	...
	if(motion_apply_win_dry(m, repeat, win, win->ui_pos, &to)){
		...
		window *win, region_t *out)
{
	...
	if(!motion_apply_win_dry(m, repeat, win, &r.begin, &r.end))
		return false;
