...
{
	to->y = UI_TOP(win) + window_nscreenlines(win) - 1;
	return MOTION_SUCCESS;
}

int m_mos(motion_arg const *m, unsigned repeat, window *win, point_t *to)
{
	to->y = UI_TOP(win) + window_nscreenlines(win) / 2 - 1;
	return MOTION_SUCCESS;
}
...
int m_dispmove(motion_arg const *m, unsigned repeat, window *win, point_t *to)
{
	if(m->pos.y)
		return m_dispmove_y(m->pos.y, repeat, win, to);
	return m_dispmove_x(m->pos.x, repeat, win, to);
}
